...
| Admin Payments & Finance | Immutable Payments, Refunds, account entries, transfers, expenses, idempotency, audit, and authoritative overview read model | Existing operational workspace plus new Finance overview and Payment detail; pagination, decomposition, and several lifecycle surfaces remain | Orders, Delivery/COD, Returns, Procurement/Landed Cost, Customers, and account postings traced | Foundation checkpoint and focused verification recorded in `areas/admin-payments-finance` | `ACTIVE_IMPLEMENTATION` |
