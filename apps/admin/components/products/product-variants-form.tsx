...
export function ProductVariantsForm({
  ...
  async function changeValueStatus(
    ...
    if (status === 'ARCHIVED' && !window.confirm('Archive this option value?')) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-foreground">Options & Variants</h2>
          <p className="text-sm text-muted-foreground">Manage your product's axes (e.g. Size, Color) and the resulting variants.</p>
        </div>
      ...
      <section className="rounded-xl border bg-card p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {activeAxes.map((axis) => (
            <div key={axis.id} className="rounded-lg border bg-muted/20 p-4">
                ...
                {axis.values.map((value) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => changeValueStatus(axis.id, value.id, value.version, value.status === 'ACTIVE' ? 'ARCHIVED' : 'ACTIVE')}
                    >
                      {value.status === 'ACTIVE' ? '×' : '↺'}
                    </button>
                  </span>
              ...
              <form onSubmit={(e) => addValue(e, axis.id)} className="flex items-center gap-2">
                <Input name="displayValue" placeholder={`New ${axis.name} value...`} className="h-8 max-w-[200px]" required />
          ...
          {activeAxes.length < 3 && (
            <form onSubmit={addAxis} className="flex items-center gap-2 rounded-lg border border-dashed p-4">
              <Input name="name" placeholder="E.g. Size, Color, Material" className="h-8 max-w-[200px]" required />
              <Button type="submit" size="sm" variant="secondary" className="h-8" disabled={busy}>
      ...
      {missingRows.length > 0 && (
        <section className="rounded-xl border border-primary/20 bg-primary/5 p-6 text-primary">
          ...
          <p className="mb-4 text-sm text-primary/80">
            You've added new options. We can automatically generate {missingRows.length} variants for you.
          </p>
